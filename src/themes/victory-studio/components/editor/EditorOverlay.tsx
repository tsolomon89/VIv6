...
interface EditorOverlayProps {
    ...
    // Layout Mode
    editorMode: 'sidebar' | 'overlay';
    setEditorMode: (mode: 'sidebar' | 'overlay') => void;
        ...
        <div className="flex flex-col border-b border-white/5 bg-white/5 shrink-0 pt-20"> 
            ...
            <div className="flex items-center justify-between px-6 py-4">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    {currentEntityId && (
                         <a 
                            href={`/dashboard?tab=pages&editId=${currentEntityId}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="p-1.5 bg-white/10 rounded-lg shrink-0 cursor-pointer hover:bg-white/20 transition-colors flex items-center justify-center" 
                            title="Open in Forms-only (Data Graph)"
                        >
                    ...
                    {/* Mode Toggle */}
                    <div 
                        className={`p-1.5 rounded-lg shrink-0 cursor-pointer transition-colors ${editorMode === 'sidebar' ? 'bg-blue-500/20 text-blue-400' : 'bg-white/10 text-white/50 hover:bg-white/20'}`}
                        onClick={() => setEditorMode(editorMode === 'sidebar' ? 'overlay' : 'sidebar')}
                        title={editorMode === 'sidebar' ? "Unpin (Overlay Mode)" : "Dock (Sidebar Mode)"}
                    >
