...
export const SectionScene = ({ objects, progress }: { objects: SceneObject[], progress: number }) => {
    ...
    const flatObjects = useMemo(() => {
        const flat = flattenScene(objects);
        return flat;
    }, [objects]);
    
    // WebGL Objects (Prism, Storm, Planes)
    const webglObjects = useMemo(() => {
        const objs = flatObjects.filter(o => isWebGLShape(o.shape));
        return objs;
    }, [flatObjects]);
    ...
    return (
        <div className="absolute inset-0 pointer-events-none z-0">
             {/* 1. WebGL Layer (Prism/Storm) - Background */}
