...
import { OblioCard3D } from '../oblio/OblioCard3D';
import { OblioScene } from '../oblio/OblioScene';

// --- Main Router ---
const DOMItem: React.FC<DOMItemProps> = (props) => {
    // Oblio Router
    if (props.obj.shape === 'card3d') {
        return <OblioCard3D obj={props.obj} />;
    }
    if (props.obj.shape === 'scene') {
        return <OblioScene obj={props.obj} scrollProgress={props.scrollProgress} />;
    }
    ...
    // Basic HTML Mappings for 'text', 'div', 'button', 'group', 'split'
    // 'split' is usually a layout container. If 'split' is used in seed as a container, 
    // we need to render it as a flexible parent.
    if (props.obj.shape === 'split') {
        // Rudimentary Split Layout Support
        const children = props.obj.children || [];
        // Assuming className handles layout (grid/flex)
        return (
            <div className={props.obj.className}>
                {children.map(child => (
                    <DOMItem 
                        key={child.id} 
                        obj={child} 
    ...
    // 'group' or 'div' - Basic container
    if (props.obj.shape === 'group' || props.obj.shape === 'div') {
         return (
             <div className={`${props.obj.className} w-full h-full`}>
                {props.obj.content && props.obj.content}
                {props.obj.children
                    ?.filter(child => isDOMShape(child.shape))
                    .map(child => (
                        <DOMItem key={child.id} obj={child} scrollProgress={props.scrollProgress} isChild={true} />
                ))}
            </div>
         );
    }
    ...
    // 'text' - Basic Text
    if (props.obj.shape === 'text') {
        // Tag could be inferred or passed
        return <div className={props.obj.className}>{props.obj.content}</div>;
    }

    // 'button'
    if (props.obj.shape === 'button') {
        return <button className={props.obj.className}>{props.obj.content}</button>;
    }

    // 'richtext' - Raw HTML (for Markdown content)
    if (props.obj.shape === 'richtext') {
        return (
            <div 
                className={`prose prose-invert max-w-none ${props.obj.className || ''}`}
                dangerouslySetInnerHTML={{ __html: props.obj.content || '' }} 
            />
        );
    }
