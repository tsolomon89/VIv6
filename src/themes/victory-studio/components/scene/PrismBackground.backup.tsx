...
export const PrismBackground: React.FC<PrismProps> = ({
  ...
  useEffect(() => {
    console.log('[PrismBackground] Mounting:', shape);
    const container = containerRef.current;
    if (!container) {
        console.error('[PrismBackground] Container Ref is NULL!');
        return;
    }
    console.log('[PrismBackground] Container found:', container.clientWidth, 'x', container.clientHeight);

    const H = Math.max(0.001, height);
    const BW = Math.max(0.001, baseWidth);

    // Derived Constants (Restored)
    const BASE_HALF = BW * 0.5;
    // Shape Mapping (Best Guess)
    const SHAPES = ['sphere', 'box', 'cylinder', 'cone', 'torus', 'prism', 'plane', 'blob', 'tetrahedron'];
    let shapeIdx = SHAPES.indexOf(shape);
    if(shapeIdx === -1) shapeIdx = 5; // Default to prism
    const SHAPE_ID = shapeIdx;
    ...
    const SCALE = scale;
    const HUE = hue;
    const CFREQ = colorFreq;
    const BLOOM = bloom;
    const RAINBOW = rainbow;
    const DENSITY = density;
    const OPACITY = opacity;
    const MARQUEE_SPEED = marqueeSpeed;
    const MARQUEE_DIR = marqueeDirection;
    const BORDER_RADIUS = borderRadius;
    const SIZING_MODE_ID = sizingMode === 'contain' ? 1 : 0;
    
    // Time/Rotation defaults
    const TS = 0.2; 
    const RSX = 1.0; 
    const RSY = 1.0; 
    const RSZ = 1.0;
    const HOVSTR = 0.5; // Hover Strength?
    const INERT = 0.95; // Inertia?
    ...
    try {
        ...
        console.error('[PrismBackground] Critical Init Error:', e);
        return;
    }
    // ... gl setup ...
    ...
    try {
        ...
        // To avoid massive indentation changes, I will use a block scope or functional extraction?
        // Actually, I can just return if renderer failed above.
        // But I need to catch other errors.
    
    // START OF WRAPPING REST OF LOGIC
    // Since replace_file_content is line-based, I can't wrap lines 115-600 easily without context.
    // I will try to catch specifics.
    
    // ...
    
    } catch (e) {
        console.error('[PrismBackground] Setup Error:', e);
        return;
    }
    ...
    let texture: any;
    try {
        console.log('[PrismBackground] Creating Texture...');
        ...
        console.error('[PrismBackground] Texture Creation Failed:', e);
        return;
    }
    ...
    try {
        console.log('[PrismBackground] Creating Geometry...');
        const geometry = new Triangle(gl);
        const iResBuf = new Float32Array(2);
        const offsetPxBuf = new Float32Array(2);

        console.log('[PrismBackground] Creating Program...');
        ...
        console.log('[PrismBackground] Creating Mesh...');
        mesh = new Mesh(gl, { geometry, program });
        console.log('[PrismBackground] Init Success');
    } catch (e) {
        console.error('[PrismBackground] GL Object Creation Failed:', e);
        return;
    }
    ...
    return () => {
      console.log('[PrismBackground] Unmounting:', shape);
      stopRAF();
      ro.disconnect();
      if (animationType === 'hover') {
