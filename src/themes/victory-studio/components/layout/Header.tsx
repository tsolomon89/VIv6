...
import { Menu, X, Power, Check, Copy, Download, LayoutTemplate, ChevronDown, PlusCircle, ShieldAlert, Lock, AlertTriangle, PanelRight, Save } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({ 
    ...
    onCopyConfig, onImportConfig, onLoadTemplate, onCreatePage, onSave, copySuccess,
    ...
    return (
      <header 
        className="fixed top-0 left-0 right-0 z-50 flex items-start justify-between py-6 mix-blend-difference text-white pointer-events-none transition-[padding] duration-300 ease-in-out"
        style={{ paddingLeft: '1.5rem', paddingRight: isSidebarOpen ? 'calc(420px + 1.5rem)' : '1.5rem' }}
      >
            ...
            <AnimatePresence>
              {hasUnlockedDebug && (
                <div className="flex flex-col gap-4 items-end">
                    ...
                    {/* Environment Status */}
                    {isDebugMode && (
                        <div className={`px-3 py-1.5 rounded-lg text-[10px] font-mono font-bold tracking-wider uppercase flex items-center gap-2 border backdrop-blur-md ${onSave ? 'bg-green-500/10 border-green-500/30 text-green-400' : 'bg-yellow-500/10 border-yellow-500/30 text-yellow-400'}`}>
                            <div className={`w-1.5 h-1.5 rounded-full ${onSave ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : 'bg-yellow-500 shadow-[0_0_8px_rgba(234,179,8,0.6)]'}`} />
                            {onSave ? 'Connected' : 'Sandbox Mode'}
                        </div>
                    )}
                    ...
                    {isDebugMode && (
                      <div className="flex items-center gap-3">
                          ...
                          {onSave && (
                              <motion.button 
                                initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2, delay: 0.1 }} 
                                onClick={onSave} 
                                className="flex items-center justify-center w-12 h-12 rounded-full transition-all duration-300 cursor-pointer bg-blue-600/80 text-white hover:bg-blue-600 shadow-lg shadow-blue-900/50" 
