...
export const heroPreset: PresentationPreset = {
    ...
    config: {
        ...
        textureUrl: "", // Background Image Support
        ...
        children: [
            base({
                ...
                glow: mkParam(5), bloom: mkParam(3), noise: mkParam(0.02),
                hueShift: mkParam(1.90), colorFrequency: mkParam(1),
                saturation: mkParam(1.1), rainbow: mkParam(1), density: mkParam(0.05),
    ...
    config: {
        ...
        children: [
             base({
                ...
                headingColor: '#ffffff',
                tileSubtitle: 'Explore our latest releases.',
                subColor: '#a3a3a3',
                subSize: mkParam(18)
            }),
            base({
                ...
                listTemplate: {
                    shape: 'card',
                    cardWidth: mkParam(380),
                    cardHeight: mkParam(500),
...
export const contentDefaultPreset: PresentationPreset = {
    key: 'content.default.v1',
    signature: { kind: 'self' },
    config: {
        ...DEFAULT_CONFIG,
        shape: 'group',
        className: 'bg-black min-h-screen', // Default background
        children: [] // Children are injected by generate.ts
    }
};

export const richTextPreset: PresentationPreset = {
    key: 'self.richtext.v1',
    signature: { kind: 'self' },
    config: {
        ...DEFAULT_CONFIG,
        shape: 'group',
        ...
        children: [
            base({
                shape: 'richtext',
                htmlContent: '<h2>Rich Text Block</h2><p>Start editing this content...</p>',
                richTextAlign: 'left',
...
// 7. TESTIMONIALS PRESET (Persona Grid)
export const testimonialGridPreset: PresentationPreset = {
    key: 'related.persona.many.grid.v1',
    signature: { kind: 'related', target: 'persona', cardinality: 'many' },
    config: {
        ...DEFAULT_CONFIG,
        className: "bg-white",
        shape: 'group',
        children: [
            base({
                ...
                offsetY: mkParam(-500),
                tileAlign: 'center',
                tileHeading: 'Trusted by Innovators.',
                headingSize: mkParam(56),
                headingColor: '#000000',
                tileSubtitle: 'Read what our partners have to say about working with us.',
                subColor: '#737373',
                subSize: mkParam(18)
            }),
            base({
                ...
                listTemplate: {
                    ...
                    headingColor: '#000000',
                    // Persona Role/Comp -> Subtitle
                    subSize: mkParam(14),
                    subColor: '#525252',
                    // Quote -> Content (Implicit mapping if entity has description/content)
                    // For now, we rely on the generic 'card' behavior rendering available fields
                    animationType: 'static'
                }
            })
        ]
...
export const PRESET_REGISTRY: PresetRegistry = {
    ...
    'related.persona.many.grid.v1': testimonialGridPreset,
    'self.cta.v1': ctaPreset,
    'section.generic.v1': genericSectionPreset,
    'content.default.v1': contentDefaultPreset,
    'self.richtext.v1': richTextPreset,
};
