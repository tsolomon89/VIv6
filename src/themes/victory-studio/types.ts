...
export type ShapeType = 
    ...
    // WebGL Effects
    'lightning' | 'atmosphere' | 'plane' |
    // DOM Primitives
    'tile' | 'card' | 'list' | 'group' |
    // Universal Primitives (Phase 17+)
    'div' | 'text' | 'button' | 'split' | 'richtext' |
    // Brand Specific
    'card3d' | 'scene';
...
export interface ConfigState {
  ...
  // Oblio / Custom Component Props
  title?: string;
  subtitle?: string;
  color?: string;
  shapeProp?: string;
  shapeVariant?: string;
  content?: string; // Generic content for text/button
  htmlContent?: string; // For Rich Text Block
  editorMinHeight?: string; // Min height for the editor area
  richTextAlign?: 'left' | 'center' | 'right' | 'justify';
}
...
export type EntityType = "brand" | "product" | "feature" | "solution" | "useCase" | "persona";
