...
export const useTemplateManager = () => {
    // Initialize from Window Config (Build), LocalStorage (Dev), or Fallback
    const [template, setTemplate] = useState<PageTemplate>(() => {
        // 1. Build Injection (Priority)
        if (typeof window !== 'undefined' && (window as any).VI_CONFIG) {
            console.log("Hydrating from VI_CONFIG");
            return (window as any).VI_CONFIG;
        }

        // 2. Local Storage (Dev persistence)
        try {
    ...
    const loadTemplate = (templateId: string) => {
        ...
        if (t) {
        if (t) {
            // Confirm disabled for dev/automation
