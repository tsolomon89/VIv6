...
import type { ConfigState, CollectionLayout, NumberParam, ShapeType } from './types';
...
export function ConfigControls({ config, onChange, headerOffset = 0, sectionHeaderClass = "" }: ConfigControlsProps) {
    ...
    return (
        <div className="space-y-8">
            ...
            {/* Advanced Material Properties (NEW) */}
            <div className="space-y-5">
                 <SectionHeader title="Advanced Materials" top={headerOffset} className={sectionHeaderClass} />
                 <ControlSlider label="Glow" param={safeParam(config.glow, 1)} min={0} max={3} step={0.1} onChange={(v) => onChange('glow', v)} />
                 <ControlSlider label="Noise" param={safeParam(config.noise, 0.5)} min={0} max={1} step={0.05} onChange={(v) => onChange('noise', v)} />
                 <ControlSlider label="Bloom" param={safeParam(config.bloom, 1)} min={0} max={3} step={0.1} onChange={(v) => onChange('bloom', v)} />
                 <ControlSlider label="Hue Shift" param={safeParam(config.hueShift, 0)} min={0} max={1} step={0.01} onChange={(v) => onChange('hueShift', v)} />
                 <ControlSlider label="Color Frequency" param={safeParam(config.colorFrequency, 1)} min={0.1} max={5} step={0.1} onChange={(v) => onChange('colorFrequency', v)} />
                 <ControlSlider label="Time Scale" param={safeParam(config.timeScale, 0.5)} min={0} max={3} step={0.1} onChange={(v) => onChange('timeScale', v)} />
                 <ControlSlider label="Inertia" param={safeParam(config.inertia, 0.05)} min={0} max={0.5} step={0.01} onChange={(v) => onChange('inertia', v)} />
                 <ControlSlider label="Rainbow" param={safeParam(config.rainbow, 1)} min={0} max={2} step={0.1} onChange={(v) => onChange('rainbow', v)} />
                 <ControlSlider label="Density" param={safeParam(config.density, 0.2)} min={0} max={1} step={0.05} onChange={(v) => onChange('density', v)} />
            </div>
        </div>
    );
}
