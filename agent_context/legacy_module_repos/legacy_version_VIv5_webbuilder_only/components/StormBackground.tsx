...
export const StormBackground: React.FC<StormProps> = ({ 
  ...
  useEffect(() => {
    ...
    // 2. Setup Renderer - Enable Alpha
    const renderer = new THREE.WebGLRenderer({ 
      antialias: false, 
      powerPreference: "high-performance",
      depth: false,
      stencil: false,
      alpha: true // Important for layering
    });
    ...
    const fragmentShader = `
      ...
      void main() {
          ...
          // --- 1. ATMOSPHERE / CLOUDS (Only if uMode == 0) ---
          if (uMode == 0) {
              ...
              // Adjust cloud density based on uCloudiness
              ...
              if (uCloudiness > 0.7) {
                bgLight = mix(bgLight, vec3(0.4, 0.45, 0.55), (uCloudiness - 0.7) * 3.3);
              }
          ...
          if (uMode == 1) {
              ...
              // Since this is additive lighting, we just use the max intensity as alpha for blending?
              // Or simple addition.
              // For a transparent overlay, we want (Col * Alpha).
              // Since lightning is additive, we can output (Color, 1.0) and use gl.blendFunc?
              // But standard alpha blending (SourceAlpha, OneMinusSourceAlpha) requires valid alpha.
              // Let's use max component as alpha approximation for glowy stuff.
    ...
    const material = new THREE.ShaderMaterial({
      ...
      depthWrite: false,
      depthTest: false,
      // Additive blending for lightning looks best, standard for atmosphere
